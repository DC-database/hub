...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // (Code to build approved/rejected lists - no change)
    let approvedHTML = '';
    let approvedTotal = 0;
    ...
    try {
        ...
        // If it fails, show an error on the receipt
    ...
    sendBtn.addEventListener('click', async () => {
        ...
        try {
            // Generate PDF
            const options = { margin: 0.25, filename: `${seriesNo}.pdf`, image: { type: 'jpeg', quality: 0.98 }, jsPDF: { unit: 'in', format: [3.35, 8], orientation: 'portrait' } };
            ...
            const phoneNumbers = ['33268492', '50992023'];
            const whatsappURL_withNumber = `https://wa.me/974${phoneNumbers[0]}?text=${encodedMessage}`;

            window.open(whatsappURL_withNumber, '_blank');
