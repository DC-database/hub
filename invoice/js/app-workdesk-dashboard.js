...
const WD_DASHBOARD_CACHE_KEY = 'IBA_WD_ACTIVE_DASHBOARD_CACHE_V2';
...
async function wdBuildDashboardTasks(options = {}) {
    ...
    // 8.9.3: Always make sure Job Records are available before building the IPC
    // dashboard group. The Active Task source can come from a short snapshot, but
    // IPC is intentionally added from Job Records, so allSystemEntries must not be stale/empty.
    if (typeof ensureAllEntriesFetched === 'function') {
        try {
            await ensureAllEntriesFetched(forceRefresh);
        } catch (e) {
            console.warn('WorkDesk dashboard could not refresh Job Records for IPC list:', e);
        }
    }
